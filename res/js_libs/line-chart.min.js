    ...
    chart: function () {
        ...
        function chart(selection) {
            ...
            selection.each(function (input) {
                // Support new structure: {data: [...], statLine: {...}, title: ...}
                ...
                // Only keep entries with valid x and y
                ...
                // If no valid data, do not draw chart
                ...
                if (title) {
                    svg.append("text")
                        ...
                        .attr("x", width / 2)
                        // Place title visibly below the top edge
                        .attr("y", -margin.top / 2 + 20)
                        .attr("text-anchor", "middle")
                        .text(title);
                }

                // --- Dynamic scale domain based on data and statLine (if present) ---
                ...
                // Draw line segments, each colored by the category color of the starting point (category optional)
                ...
                // --- Statistical line and bundles (optional) ---
                if (statLine && typeof statLine === "object" &&
                    ...
                    // Draw only one dot at start and one at end, but show n_start/n_end in legend
                ...
                // Candidate positions: [x, y]
                const positions = [
                    [pad, pad], // top-left
                    [width - legendWidth - pad, pad], // top-right
                    [pad, height - legendHeight - pad], // bottom-left
                    [width - legendWidth - pad, height - legendHeight - pad] // bottom-right
                ];
                // Score each position: prefer those not overlapping data
                function overlaps(x0, y0) {
                    // Check if legend box overlaps data bounding box (with some margin)
                    const box = {x1: x0, y1: y0, x2: x0 + legendWidth, y2: y0 + legendHeight};
                    const dataBox = {x1: minX - 10, y1: minY - 10, x2: maxX + 10, y2: maxY + 10};
                    // If legend box is completely outside data box, it's good
                    return !(box.x2 < dataBox.x1 || box.x1 > dataBox.x2 || box.y2 < dataBox.y1 || box.y1 > dataBox.y2);
                }
                let legendPos = positions.find(pos => !overlaps(pos[0], pos[1])) || positions[1]; // fallback: top-right
                ...
                if (statLine && statLine.start_x !== undefined && statLine.start_y !== undefined &&
                    ...
                    legend.append("line")
                        ...
                        .attr("stroke-width", 3)
                        .attr("stroke-dasharray", "8,4");
                    // Show only one dot for start and one for end, with label for how many points they represent
                    let pointSize = statLine.pointSize ? +statLine.pointSize : 6;
                    let color = statLine.color || "#d62728";
